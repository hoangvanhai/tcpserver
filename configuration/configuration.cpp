...
bool userconfig::load_config()
{
    ...
    {
        ftp_server_info def = {"/opt/data/ftp1/", "127.0.0.1", "FTP-User", "123456a@", 21,  1000, 5, 100000};
    ...
    {
        ftp_server_info def = {"/opt/data/ftp2/", "127.0.0.1", "FTP-User", "123456a@", 21,  1000, 5, 100000};
        ...
        LREP("ftp_server: address: {}, port: {}, username: {}, passwd: {}, scan: {}, log: {} max_hold: {}\n",
             ...
             config_.server2.scan_dur, config_.server2.log_dur, config_.server2.max_hold);
    }
...
void userconfig::load_ftp_server_config(ftp_server_info &server, ftp_server_info &def_val, int idx)
{
    Json::Value value;
    bool retVal;

    retVal = try_get_object(root_, "ftp_server" + std::to_string(idx), value);
...
void userconfig::save_ftp_server_config(const ftp_server_info &server, int idx)
{
    Json::Value value;
...
void userconfig::save_user_config(const user_config &cfg)
{
    ...
    save_ftp_server_config(cfg.server2, 2);
    for(int i = 0; i < APP_SYSTEM_MAX_TAG; i++) {
        if(i <= 7) {
